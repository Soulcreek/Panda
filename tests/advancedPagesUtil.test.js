...
describe('Advanced Pages Utilities', () => {
      ...
      rows: [
        {
          columns: [
            {
              width: 12,
              blocks: [{ type: 'hero', title: 'Titel', subtitle: 'Sub', height: '40vh' }],
            },
          ],
        },
      ],
  ...
  test('podcast-link block renders link card when helper provided', () => {
    const layout = {
      rows: [{ columns: [{ width: 12, blocks: [{ type: 'podcast-link', podcastId: 5 }] }] }],
    };
    const html = renderAdvancedLayout(layout, {
      podcasts: { 5: { id: 5, title: 'Episode 5', description: 'Desc' } },
    });
