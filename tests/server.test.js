...
let adminRoutes;
try { adminRoutes = require('../routes/admin'); } catch { adminRoutes = express.Router(); }

// Editors modular router
let editorsRouter;
try { editorsRouter = require('../routes/editors/index'); } catch { editorsRouter = express.Router(); }
...
function buildApp(){
  ...
  a.use((req,res,next)=>{
    ...
    for(const [re,target] of rules){ if(re.test(req.path)) return res.redirect(301,target); }
    next();
  });
