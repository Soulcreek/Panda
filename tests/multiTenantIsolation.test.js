...
describe('Multi-tenant slug isolation', ()=>{
  ...
  test('same slug allowed on different sites', async()=>{
    ...
    await pool.query('INSERT INTO '+table+' (title, slug, site_key) VALUES (?,?,?)',[slug, slug, siteA]);
    await pool.query('INSERT INTO '+table+' (title, slug, site_key) VALUES (?,?,?)',[slug, slug, siteB]);
    const [rows] = await pool.query('SELECT site_key, slug FROM '+table+' WHERE slug=? AND site_key IN (?,?)',[slug, siteA, siteB]);
