...
function extractJson(raw) {
  ...
  if (fenced) {
    work = fenced[0].replace(/```json|```/gi, '').trim();
  }
  // First direct parse attempt
  let parsed = null;
  let error = null;
  try {
    parsed = JSON.parse(work);
  } catch (e) {
    error = e.message;
  }
  ...
  if (first >= 0 && last > first) {
    const slice = work.slice(first, last + 1);
    try {
      parsed = JSON.parse(slice);
      return { text: slice, json: parsed, error: null };
    } catch (e2) {
      error = e2.message;
    }
  }
  ...
  if (/\}\s*```/.test(work)) {
    const cleaned = work.replace(/```.*$/s, '');
    try {
      parsed = JSON.parse(cleaned);
      return { text: cleaned, json: parsed, error: null };
    } catch (e3) {
      error = e3.message;
    }
  }
...
function validateShape(obj, shape) {
  ...
  for (const [k, type] of Object.entries(shape)) {
    ...
    if (type === 'array') {
      if (!Array.isArray(obj[k])) errors.push('type_' + k);
