...
async function ensureAdvancedPagesTables() {
  try {
    ...
    try {
      await pool.query('ALTER TABLE advanced_pages ADD COLUMN seo_title VARCHAR(255)');
    } catch (_) {}
    try {
      await pool.query('ALTER TABLE advanced_pages ADD COLUMN seo_description VARCHAR(255)');
    } catch (_) {}
    try {
      await pool.query('ALTER TABLE advanced_pages ADD COLUMN meta_keywords VARCHAR(255)');
    } catch (_) {}
    try {
      await pool.query('ALTER TABLE advanced_pages ADD COLUMN meta_image VARCHAR(255)');
    } catch (_) {}
    try {
      await pool.query(
        'ALTER TABLE advanced_pages ADD COLUMN site_key VARCHAR(64) NOT NULL DEFAULT "default"'
      );
    } catch (_) {}
    try {
      await pool.query('ALTER TABLE advanced_pages ADD INDEX idx_adv_pages_site (site_key)');
    } catch (_) {}
    // Adjust uniqueness: old schema had UNIQUE on slug (single tenant). Replace with composite (site_key, slug)
    try {
      await pool.query('ALTER TABLE advanced_pages DROP INDEX slug');
    } catch (_) {}
    try {
      await pool.query(
        'ALTER TABLE advanced_pages ADD UNIQUE INDEX uq_adv_pages_site_slug (site_key, slug)'
      );
    } catch (_) {}
    ...
    try {
      await pool.query(
        'ALTER TABLE advanced_page_generation_logs ADD COLUMN site_key VARCHAR(64) NOT NULL DEFAULT "default"'
      );
    } catch (_) {}
    try {
      await pool.query(
        'ALTER TABLE advanced_page_generation_logs ADD INDEX idx_adv_gen_site (site_key)'
      );
    } catch (_) {}
  } catch (e) {
    console.warn('[advancedPagesUtil] ensure tables failed', e.message);
  }
}
  ...
  while (true) {
    try {
      const [rows] = await pool.query(
        'SELECT id FROM advanced_pages WHERE slug=? AND id<>? AND site_key=? LIMIT 1',
        [final, id || 0, key]
      );
      if (!rows.length) return final;
    } catch (e) {
      // Fallback (older schema without site_key)
      const [rows] = await pool.query('SELECT id FROM advanced_pages WHERE slug=? AND id<>?', [
        final,
        id || 0,
      ]);
      if (!rows.length) return final;
    ...
    for (const col of row.columns) {
      ...
      if (Array.isArray(col.blocks)) {
        for (const b of col.blocks) {
          if (b.type === 'image') {
            html +=
              '<figure class="ap-block-image"><img src="' +
              escapeHtml(b.src || '') +
              '" alt="' +
              escapeHtml(b.alt || '') +
              '" class="img-fluid"/>' +
              (b.caption
                ? '<figcaption class="small text-muted">' + escapeHtml(b.caption) + '</figcaption>'
                : '') +
              '</figure>';
          } else if (b.type === 'background') {
            html +=
              '<div class="ap-block-bg" style="background:' +
              (b.bgColor || '#f5f5f5') +
              ';padding:' +
              (b.padding || '20px') +
              ';">' +
            ...
            html +=
              '<div class="ap-hero" style="position:relative;min-height:' +
              (b.height || '50vh') +
              ...
              (b.image
                ? '<img src="' +
                  escapeHtml(b.image) +
                  '" alt="" style="object-fit:cover;position:absolute;top:0;left:0;width:100%;height:100%;opacity:' +
                  (b.overlayOpacity ? Math.max(0, Math.min(1, b.overlayOpacity)) : 0.4) +
                  ';filter:brightness(0.6);"/>'
                : '') +
              '<div class="container position-relative"><h1 class="display-5">' +
              escapeHtml(b.title || '') +
              '</h1><p class="lead">' +
            ...
            if (p) {
              html +=
                '<div class="ap-post-link card h-100"><div class="card-body"><h3 class="h5">' +
                escapeHtml(p.title) +
                '</h3><p class="small text-muted">' +
                escapeHtml((p.whatsnew || '').slice(0, 140)) +
                '</p><a href="/blog/' +
                escapeHtml(p.slug || '') +
                '" class="stretched-link">Mehr</a></div></div>';
            }
          } else if (b.type === 'podcast-link' && b.podcastId && helpers && helpers.podcasts) {
            const pod = helpers.podcasts[b.podcastId];
            if (pod) {
              html +=
                '<div class="ap-podcast-link card h-100"><div class="card-body"><h3 class="h6">' +
                escapeHtml(pod.title) +
                '</h3><p class="small text-muted">' +
                escapeHtml((pod.description || '').slice(0, 140)) +
                '</p><a href="/podcasts/' +
                pod.id +
                '" class="stretched-link">Anhören</a></div></div>';
            }
          }
        }
      }
