...
async function loadAiConfig() {
  if (!loadAiConfig.cache || Date.now() - loadAiConfig.cacheTime > 15000) {
    const cfg = { primary_key_choice: 'paid', max_daily_calls: 500 };
    try {
      const [rows] = await pool.query(
        'SELECT primary_key_choice, max_daily_calls FROM ai_config WHERE id=1'
      );
      if (rows.length) {
        cfg.primary_key_choice = rows[0].primary_key_choice || cfg.primary_key_choice;
        cfg.max_daily_calls = rows[0].max_daily_calls || cfg.max_daily_calls;
      }
    } catch (_) {}
    loadAiConfig.cache = cfg;
    loadAiConfig.cacheTime = Date.now();
  }
...
async function callGeminiRaw({
  ...
  if (metrics) {
    ...
    metrics.observe('ai_call_duration_seconds', dur);
  }
  if (!res.ok) {
  ...
  if (json) {
    ...
    if (match) {
      text = match[0].replace(/```json|```/gi, '').trim();
    }
  }
...
async function loadPrompts() {
  ...
  if (!loadPrompts.cache || Date.now() - loadPrompts.cacheTime > 30000) {
    ...
    try {
      ...
      if (rows.length && rows[0].prompts) {
        try {
          prompts = JSON.parse(rows[0].prompts) || {};
        } catch (_) {}
      }
    } catch (_) {}
    loadPrompts.cache = prompts;
    loadPrompts.cacheTime = Date.now();
  }
...
async function generateWhatsNew() {
  ...
  const baseResearch =
    prompts.whats_new_research ||
    'Liste in 5-7 knappen Bullet-Points die wichtigsten aktuellen News (max 400 Zeichen total).';
  const stage1Sys =
    'You are an analyst summarizing latest developments in data governance, Microsoft Purview, security & compliance (German).';
  const stage1User = baseResearch;
  const s1 = await callGeminiRaw({
    system: stage1Sys,
    user: stage1User,
    model: modelFast,
    temperature: 0.5,
  ...
  const stage2Sys =
    'You are a German tech blogger. Create an original concise blog post (HTML) based on provided bullet points.';
  const stage2User = `Nutze diese Bullet Points:\n${bullets}\n\nAntwort als JSON mit Feldern: {"title_de":"...","content_de":"<p>HTML …</p>"}`;
  const s2 = await callGeminiRaw({
    system: stage2Sys,
    user: stage2User,
    model: modelHeavy,
    json: true,
  ...
  return {
    stage1: bullets,
    title_de: parsed.json ? parsed.json.title_de : undefined,
    content_de: parsed.json ? parsed.json.content_de : undefined,
    raw: s2.text,
    parse_error: !parsed.shapeOk ? parsed.shapeErrors.join(',') || parsed.error : null,
  };
}
...
async function generateSampleArticle() {
  ...
  const custom =
    prompts.blog_sample ||
    'Erzeuge Beispiel Blogartikel als JSON {"title_de":"...","content_de":"<p>HTML…</p>"} zum Thema Microsoft Purview Grundlagen. Kurz (ca. 4 Absätze).';
...
async function generatePodcastMetadata(title, description) {
  ...
  const parsed = parseWithShape(r.text, {
    seo_title: 'string',
