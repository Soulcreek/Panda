...
const ALLOWED_MIME = [
  ...
  'audio/mpeg','audio/mp3','audio/wav','audio/ogg','video/mp4','application/pdf'
];
...
function validateBasic(file){
  ...
  if(file.size > MAX_FILE_SIZE_BYTES){
    errors.push(`Datei zu groß: ${(file.size/1024/1024).toFixed(2)}MB > ${(MAX_FILE_SIZE_BYTES/1024/1024)}MB`);
  }
  return errors;
}
...
function validateImageDimensions(filePath){
  try {
    ...
    if(dim.width > MAX_IMAGE_WIDTH || dim.height > MAX_IMAGE_HEIGHT){
      return [`Bildmaße zu groß: ${dim.width}x${dim.height} > ${MAX_IMAGE_WIDTH}x${MAX_IMAGE_HEIGHT}`];
    }
    return [];
  } catch(e){
...
module.exports = { validateUploadedFile, ALLOWED_MIME, MAX_FILE_SIZE_BYTES, MAX_IMAGE_WIDTH, MAX_IMAGE_HEIGHT };
