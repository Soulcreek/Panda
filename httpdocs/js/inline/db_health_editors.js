(function(){
  ...
  async function ping(force){ try { const r = await fetch('/admin/api/db-health'+(force?'?force=1':'') ,{headers:{'Accept':'application/json'}}); const d= await r.json(); if(d){ updateBanner(d); } } catch(_){ }
  }
