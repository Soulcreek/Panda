...
(function () {
  ...
  const builderEl = document.getElementById('apBuilder');
  if (!builderEl) return;
  ...
  const history = {
    ...
    push(state) {
      ...
      if (this.index >= 0 && this.stack[this.index] === s) return;
  ...
  function addBlock(rowIndex, colIndex, type) {
    ...
    switch (type) {
      case 'image':
        block = { id, type: 'image', src: '/uploads/Panda_Banner.png', alt: '', caption: '' };
        break;
      case 'background':
        block = {
          id,
          ...
          bgColor: '#f5f5f5',
          padding: '40px 20px',
          html: '<h3>Section Titel</h3><p>Inhalt…</p>',
        };
        break;
      case 'text':
        block = {
      ...
      case 'hero':
        ...
        break;
      default:
        block = { id, type: 'html', html: '<p>Neuer Block</p>' };
    }
    col.blocks.push(block);
    render();
    capture();
    setTimeout(() => {
      const el = builderEl.querySelector(`[data-block='${id}'] .ap-block-editable`);
      if (el) {
        el.focus();
        selectAll(el);
      }
  ...
  function moveRow(rowIndex, dir) {
    const ni = rowIndex + dir;
    if (ni < 0 || ni >= layout.rows.length) return;
    const tmp = layout.rows[rowIndex];
    layout.rows[rowIndex] = layout.rows[ni];
    layout.rows[ni] = tmp;
    render();
    capture();
  }
  ...
  function render() {
    ...
    layout.rows.forEach((row, rowIndex) => {
      ...
      rowWrap.className = 'ap-row position-relative mb-4';
      const rowToolbar = document.createElement('div');
      rowToolbar.className = 'ap-builder-row-toolbar';
      rowToolbar.innerHTML = `<div class="btn-group btn-group-sm"><button type="button" class="btn btn-outline-secondary ap-row-up" ${rowIndex === 0 ? 'disabled' : ''}>&uarr;</button><button type="button" class="btn btn-outline-secondary ap-row-down" ${rowIndex === layout.rows.length - 1 ? 'disabled' : ''}>&darr;</button><button type="button" class="btn btn-outline-danger ap-row-del">X</button></div> <span class="badge bg-info ap-mini-badge">${row.preset}</span>`;
      rowWrap.appendChild(rowToolbar);
      const rowContent = document.createElement('div');
      rowContent.className = 'row g-3';
      row.columns.forEach((col, colIndex) => {
        const colDiv = document.createElement('div');
        colDiv.className = `col-md-${col.width}`;
        const inner = document.createElement('div');
        inner.className = 'ap-builder-col';
        col.blocks.forEach((block) => {
          const blockDiv = document.createElement('div');
          blockDiv.className = 'ap-block';
          blockDiv.setAttribute('data-block', block.id);
          let bodyHTML = '';
          if (block.type === 'image') {
            bodyHTML = `<div class='ap-block-image-wrapper text-center'><img src='${block.src || ''}' alt='${block.alt || ''}' style='max-width:100%;height:auto;'/><div class='small text-muted mt-1'>${block.caption || ''}</div></div>`;
          } else if (block.type === 'background') {
            bodyHTML = `<div class='ap-block-bg-inner' style='background:${block.bgColor || '#f5f5f5'};padding:${block.padding || '20px'};'><div class='ap-block-editable' contenteditable='true'>${block.html || ''}</div></div>`;
          } else if (block.type === 'post-link') {
            bodyHTML = `<div class='ap-block-link-preview'><div class='small text-muted'>Post Link</div><strong>${escapeHtml(block.title || '(kein Titel)')}</strong><div class='text-muted small'>${escapeHtml(block.slug || '')}</div></div>`;
          } else if (block.type === 'podcast-link') {
            bodyHTML = `<div class='ap-block-link-preview'><div class='small text-muted'>Podcast Link</div><strong>${escapeHtml(block.title || '(kein Titel)')}</strong></div>`;
          } else if (block.type === 'hero') {
            bodyHTML = `<div class='ap-block-hero-preview text-center p-3' style='background:#222;color:#fff;min-height:140px;display:flex;flex-direction:column;justify-content:center;'><div class='small text-uppercase text-muted'>Hero</div><div class='fw-bold'>${escapeHtml(block.title || 'Hero Titel')}</div><div class='small'>${escapeHtml(block.subtitle || '')}</div></div>`;
          } else {
            const styleInline = `${block.textColor ? `color:${block.textColor};` : ''}${block.bgColor ? `background:${block.bgColor};` : ''}`;
            bodyHTML = `<div class='ap-block-editable ${block.spacing || ''} ${block.hideSM ? 'd-none d-sm-block' : ''} ${block.hideMD ? 'd-sm-none d-md-block' : ''} ${block.hideLG ? 'd-lg-none' : ''}' contenteditable='true' style='${styleInline}'>${block.html || ''}</div>`;
          }
          blockDiv.innerHTML = `<div class='ap-block-toolbar d-flex flex-wrap gap-1'><span class='badge bg-secondary'>${block.type}</span><div class='btn-group btn-group-sm me-auto'><button type='button' class='btn btn-outline-secondary ap-block-collapse' title='Ein-/Ausblenden'>&minus;</button><button type='button' class='btn btn-outline-danger ap-block-delete' title='Löschen'>&times;</button></div>${block.type === 'html' || block.type === 'text' ? `<div class='btn-group btn-group-sm ap-mini-format' data-block='${block.id}'><button type='button' data-cmd='bold' class='btn btn-outline-secondary'>B</button><button type='button' data-cmd='italic' class='btn btn-outline-secondary'><em>I</em></button><button type='button' data-cmd='h2' class='btn btn-outline-secondary'>H2</button><button type='button' data-cmd='ul' class='btn btn-outline-secondary'>&bull; List</button></div>` : ''}<div class='btn-group btn-group-sm ap-quick-actions'><button type='button' class='btn btn-outline-secondary ap-block-move-up' title='Move Up'>&uarr;</button><button type='button' class='btn btn-outline-secondary ap-block-move-down' title='Move Down'>&darr;</button><button type='button' class='btn btn-outline-secondary ap-block-clone' title='Clone'>⧉</button></div><button type='button' class='btn btn-sm btn-outline-info ap-block-config' data-block='${block.id}'>Config</button></div><div class='ap-block-content'>${bodyHTML}</div>`;
          inner.appendChild(blockDiv);
        });
        const addBtn = document.createElement('div');
        addBtn.className = 'd-flex gap-2 flex-wrap mt-1';
        ['html', 'text', 'image', 'background', 'post-link', 'podcast-link', 'hero'].forEach(
          (tp) => {
            const b = document.createElement('button');
            b.type = 'button';
            b.className = 'btn btn-sm btn-outline-primary';
            b.textContent = '+ ' + (i18n['add.' + tp] || tp);
            b.addEventListener('click', () => addBlock(rowIndex, colIndex, tp));
            addBtn.appendChild(b);
          }
        );
        inner.appendChild(addBtn);
        colDiv.appendChild(inner);
        rowContent.appendChild(colDiv);
      });
      ...
      rowToolbar
        .querySelector('.ap-row-down')
        .addEventListener('click', () => moveRow(rowIndex, 1));
  ...
  builderEl.addEventListener('click', (e) => {
    const delBtn = e.target.closest('.ap-block-delete');
    if (delBtn) {
      const blk = delBtn.closest('.ap-block');
      if (!blk) return;
      const id = blk.getAttribute('data-block');
      layout.rows.forEach((r, ri) =>
        r.columns.forEach((c, ci) => {
          if (c.blocks.find((b) => b.id === id)) deleteBlock(ri, ci, id);
        })
      );
      return;
    }
    const colBtn = e.target.closest('.ap-block-collapse');
    if (colBtn) {
      const blk = colBtn.closest('.ap-block');
      const content = blk.querySelector('.ap-block-content');
      if (!content) return;
      const hidden = content.style.display === 'none';
      content.style.display = hidden ? '' : 'none';
      return;
    }
    const cloneBtn = e.target.closest('.ap-block-clone');
    if (cloneBtn) {
      const id = cloneBtn.closest('.ap-block').getAttribute('data-block');
      cloneBlockById(id);
      return;
    }
    const mvUp = e.target.closest('.ap-block-move-up');
    if (mvUp) {
      moveBlock(mvUp.closest('.ap-block').getAttribute('data-block'), -1);
      return;
    }
    const mvDown = e.target.closest('.ap-block-move-down');
    if (mvDown) {
      moveBlock(mvDown.closest('.ap-block').getAttribute('data-block'), 1);
      return;
    }
    const fmtBtn = e.target.closest('.ap-mini-format button');
    if (fmtBtn) {
      const cmd = fmtBtn.getAttribute('data-cmd');
      const blockId = fmtBtn.closest('.ap-mini-format').dataset.block;
      const blkEl = builderEl.querySelector(
        `.ap-block[data-block='${blockId}'] .ap-block-editable`
      );
      if (!blkEl) return;
      blkEl.focus();
    ...
    const cfgBtn = e.target.closest('.ap-block-config');
    if (cfgBtn) {
      openConfig(cfgBtn.getAttribute('data-block'));
      return;
    }
  });
  ...
  function openConfig(blockId) {
    let targetBlock;
    outer: for (const r of layout.rows) {
      for (const c of r.columns) {
        const f = c.blocks.find((b) => b.id === blockId);
        if (f) {
          targetBlock = f;
          break outer;
        }
    ...
    panel.addEventListener('change', () => {
      ...
      } else if (targetBlock.type === 'html' || targetBlock.type === 'text') {
        targetBlock.textColor = panel.querySelector('.ap-cfg-textcolor').value;
        targetBlock.bgColor = panel.querySelector('.ap-cfg-bgc').value;
        targetBlock.classes = panel.querySelector('.ap-cfg-classes').value.trim();
        targetBlock.spacing = panel.querySelector('.ap-cfg-spacing').value;
        targetBlock.hideSM = panel.querySelector('.ap-vis-hide-sm').checked;
        targetBlock.hideMD = panel.querySelector('.ap-vis-hide-md').checked;
        targetBlock.hideLG = panel.querySelector('.ap-vis-hide-lg').checked;
      } else if (targetBlock.type === 'hero') {
        targetBlock.image = panel.querySelector('.ap-cfg-hero-image').value.trim();
        targetBlock.title = panel.querySelector('.ap-cfg-hero-title').value;
        targetBlock.subtitle = panel.querySelector('.ap-cfg-hero-sub').value;
        targetBlock.ctaText = panel.querySelector('.ap-cfg-hero-cta-text').value;
        targetBlock.ctaUrl = panel.querySelector('.ap-cfg-hero-cta-url').value;
        targetBlock.overlayOpacity =
          parseFloat(panel.querySelector('.ap-cfg-hero-overlay').value) || 0;
        targetBlock.height = panel.querySelector('.ap-cfg-hero-height').value.trim() || '60vh';
        targetBlock.align = panel.querySelector('.ap-cfg-hero-align').value;
      }
      render();
      capture();
    });
    if (targetBlock.type === 'image') {
      panel.querySelector('.ap-cfg-pick').addEventListener('click', () => {
        if (window.Pickers) {
          Pickers.media({ type: 'image' })
            .then((item) => {
              targetBlock.src = item.path;
              if (item.alt_text) targetBlock.alt = item.alt_text;
              render();
              capture();
            })
            .catch(() => {});
      ...
      panel.querySelector('.ap-cfg-pick-hero').addEventListener('click', () => {
        if (window.Pickers) {
          Pickers.media({ type: 'image' })
            .then((item) => {
              targetBlock.image = item.path;
              render();
              capture();
            })
            .catch(() => {});
    ...
    } else if (targetBlock.type === 'post-link') {
      panel.querySelector('.ap-cfg-pick-post').addEventListener('click', () => {
        if (window.Pickers) {
          Pickers.post()
            .then((p) => {
              targetBlock.postId = p.id;
              targetBlock.title = p.title;
              targetBlock.slug = p.slug;
              targetBlock.excerpt = p.excerpt || '';
              render();
              capture();
            })
            .catch(() => {});
    ...
    } else if (targetBlock.type === 'podcast-link') {
      panel.querySelector('.ap-cfg-pick-podcast').addEventListener('click', () => {
        if (window.Pickers) {
          Pickers.podcast()
            .then((p) => {
              targetBlock.podcastId = p.id;
              targetBlock.title = p.title;
              targetBlock.description = p.description || '';
              render();
              capture();
            })
            .catch(() => {});
  ...
  jsonToggleBtn &&
    jsonToggleBtn.addEventListener('click', () => {
      jsonWrapper.classList.toggle('d-none');
  ...
  document
    .querySelectorAll('.ap-add-row')
    .forEach((btn) => btn.addEventListener('click', () => addRow(btn.dataset.preset)));
  ...
  function moveBlock(blockId, dir) {
    const pos = findBlockPosition(blockId);
    if (!pos) return;
    const col = layout.rows[pos.row].columns[pos.col];
    const ni = pos.index + dir;
    if (ni < 0 || ni >= col.blocks.length) return;
    const [blk] = col.blocks.splice(pos.index, 1);
    col.blocks.splice(ni, 0, blk);
    render();
    capture();
  }
  ...
  builderEl.addEventListener('dragstart', (e) => {
    ...
    const id = blk.getAttribute('data-block');
    const pos = findBlockPosition(id);
    if (!pos) return;
    dragData = pos;
  ...
  builderEl.addEventListener('dragend', (e) => {
    const blk = e.target.closest('.ap-block');
    if (blk) blk.classList.remove('ap-block-dragging');
    document
      .querySelectorAll('.ap-block-drag-over')
      .forEach((x) => x.classList.remove('ap-block-drag-over'));
  });
  builderEl.addEventListener('dragover', (e) => {
    const blk = e.target.closest('.ap-block');
    if (blk) {
      e.preventDefault();
      blk.classList.add('ap-block-drag-over');
    }
  });
  builderEl.addEventListener('dragleave', (e) => {
    const blk = e.target.closest('.ap-block');
    if (blk) blk.classList.remove('ap-block-drag-over');
  });
  builderEl.addEventListener('drop', (e) => {
    const targetBlk = e.target.closest('.ap-block');
    if (!targetBlk || !dragData) return;
    e.preventDefault();
    ...
    if (dragData.row === targetPos.row && dragData.col === targetPos.col) {
      ...
      if (dragData.index < targetPos.index) insertIndex--;
  ...
  document.addEventListener('keydown', (e) => {
    if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 's') {
      ...
      syncHidden();
      try {
        document.getElementById('advancedPageForm').submit();
      } catch (_) {}
    } else if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'z') {
      e.preventDefault();
      const prev = history.undo();
      if (prev) {
        layout.rows = prev.rows;
        render();
      }
    } else if (
      (e.ctrlKey || e.metaKey) &&
  ...
  if (controlsBar) {
    ...
    undo.type = 'button';
    undo.className = 'btn btn-sm btn-outline-secondary ap-btn-undo';
    undo.textContent = i18n.undo || 'Undo';
    const redo = document.createElement('button');
    redo.type = 'button';
    redo.className = 'btn btn-sm btn-outline-secondary ap-btn-redo';
    redo.textContent = i18n.redo || 'Redo';
    const tpl = document.createElement('select');
    tpl.className = 'form-select form-select-sm w-auto ap-template-select';
    const templates = {
      'hero+two-cols': {
        version: 2,
        rows: [
                ...
                blocks: [
                  {
                    ...
                    image: '/uploads/Panda_Banner.png',
                    ctaText: 'Mehr',
                    ctaUrl: '#',
                    overlayOpacity: 0.5,
                    height: '60vh',
                ...
                blocks: [
                  {
                    id: 't' + (Date.now() + 1),
                    type: 'text',
                    html: '<h3>Spalte 1</h3><p>Inhalt…</p>',
                  },
                ],
              },
              {
                ...
                blocks: [
                  {
                    id: 't' + (Date.now() + 2),
                    type: 'text',
                    html: '<h3>Spalte 2</h3><p>Inhalt…</p>',
                  },
                ],
              },
            ],
            ...
            columns: [
              {
                width: 4,
                blocks: [
                  { id: 't' + Date.now(), type: 'post-link', title: '(Post 1)', postId: null },
                ],
              },
              {
                width: 4,
                ...
                blocks: [
                  {
                    id: 't' + (Date.now() + 3),
                    type: 'text',
                    html: '<div class="text-center"><a class="btn btn-primary btn-lg" href="#">Call To Action</a></div>',
                  },
                ],
              },
            ],
    ...
    tpl.innerHTML =
      ...
      Object.keys(templates)
        ...
        .join('');
    controlsBar.appendChild(undo);
    controlsBar.appendChild(redo);
    controlsBar.appendChild(tpl);
    ...
    tpl.addEventListener('change', () => {
      const key = tpl.value;
      if (!key) return;
      if (layout.rows.length && !confirm('Template anwenden und aktuelles Layout ersetzen?')) {
        tpl.value = '';
        return;
      }
      const snap = JSON.parse(JSON.stringify(templates[key]));
      layout.rows = snap.rows;
      render();
      capture();
      tpl.value = '';
    });
  ...
  function updateUndoRedoButtons() {
    const u = document.querySelector('.ap-btn-undo');
    const r = document.querySelector('.ap-btn-redo');
    if (!u || !r) return;
