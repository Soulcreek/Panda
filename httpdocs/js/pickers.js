/* Unified Pickers (Media, Post, Podcast) using A11yDialog
 * Provides promise-based selection APIs returning chosen entity object.
 * Usage:
 *   Pickers.media({ type:'image' }).then(item=>{ ... });
 *   Pickers.post().then(post=>{ ... });
 *   Pickers.podcast().then(podcast=>{ ... });
 */
(function(){
  if(window.Pickers) return;
  const BASE = (location.pathname.startsWith('/editors')) ? '/editors' : '/admin';
  async function apiFetchJSON(url){
    try { return await apiFetch(url, { headers:{ 'Accept':'application/json' }}); }
    catch(e){ throw e; }
  }
