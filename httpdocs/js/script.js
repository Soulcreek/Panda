...
async function openMediaLibraryModal(options) {
  ...
  try {
    ...
    mediaItems.forEach((item) => {
      ...
      card.className =
        'card h-100 media-library-item d-flex align-items-center justify-content-center';
      ...
      if (item.type && item.type.startsWith('image/')) {
        ...
        const filename =
          item.name || item.filename || (item.path ? item.path.split('/').pop() : '');
        ...
        wrap.innerHTML =
          '<i class="bi bi-music-note-beamed fs-2"></i><div class="small text-muted text-truncate mw-100">' +
          filename +
          '</div>';
        ...
        wrap.className = 'text-center p-3';
        wrap.innerHTML =
          '<i class="bi bi-file-earmark fs-2"></i><div class="small text-muted">' +
          filename +
          '</div>';
        card.appendChild(wrap);
      }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    blogPostModalEl.addEventListener('show.bs.modal', async (event) => {
      ...
      try {
        ...
        if (!post || !post.content) {
          console.warn('Leerer Post API Response', post);
        }
        ...
        modalContent.innerHTML =
          '<p class="text-danger">Der Beitrag konnte nicht geladen werden (' +
          error.message +
          ').</p>';
        const dbg = document.createElement('pre');
        dbg.className = 'small text-muted';
        dbg.textContent = 'ID ' + postId + ' Fehler: ' + error.message;
        modalContent.appendChild(dbg);
      }
    });
  }
  ...
  try {
    ...
    console.warn('Media form binding failed', e);
  }
  ...
  if (generateBtn) {
    generateBtn.addEventListener('click', async function () {
      ...
      try {
        const data = await apiFetch('/admin/generate-whats-new', {
          method: 'POST',
          headers: { 'CSRF-Token': window.CSRF_TOKEN || '' },
        });
        document.getElementById('title_de').value = data.title_de;
        const elDe = document.getElementById('content_de');
        if (elDe) elDe.value = data.content_de;
        document.getElementById('title_en').value = data.title_en;
        const elEn = document.getElementById('content_en');
        if (elEn) elEn.value = data.content_en;
      } catch (error) {
        ...
        const fbContent =
          '<p>Dieser Inhalt wurde als Fallback generiert, da der KI-Dienst nicht verfügbar war. Kurzer Überblick über aktuelle Trends: Zero Trust, Datenklassifizierung, automatisierte Compliance-Überwachung.</p>';
        alert(
          'Der Inhalt konnte nicht generiert werden (' +
            error.message +
            '). Fallback verwendet: ' +
            fallbackSource
        );
        const elDe2 = document.getElementById('content_de');
        if (elDe2) {
          document.getElementById('title_de').value = fbTitle;
          elDe2.value = fbContent;
        }
      } finally {
        this.disabled = false;
  ...
  if (generateAltTextBtn && mediaFilesInput) {
    ...
    function updateBtnState() {
      ...
      if (hint) {
        if (count === 1) {
          hint.textContent = 'Bereit: KI kann ALT & Beschreibung generieren.';
        } else if (count > 1) {
          hint.textContent = 'Mehrfachauswahl: KI deaktiviert (wähle genau 1 Datei).';
        } else {
          hint.textContent = 'Wähle genau eine Bilddatei für KI ALT-Generierung.';
        }
      }
  ...
  if (mediaTypeFilter) {
    mediaTypeFilter.addEventListener('click', (e) => {
      ...
      document.querySelectorAll('.media-grid-item').forEach((card) => {
        ...
        if (t === 'all' || (t === 'image' && isImage) || (t === 'audio' && isAudio))
          card.style.display = '';
  ...
  document.addEventListener('click', async function (e) {
    ...
    if (target.classList.contains('toggle-visibility-btn')) {
      ...
      const resp = await apiFetch(`/admin/post/toggle-visibility/${postId}`, {
        method: 'POST',
        csrf: true,
      });
    ...
    if (target.classList.contains('delete-post-btn')) {
      if (
        confirm('Sind Sie sicher, dass Sie diesen Beitrag löschen möchten? Er wird nur archiviert.')
      ) {
        const postId = target.dataset.id;
    ...
    if (target.classList.contains('toggle-featured-btn')) {
      const postId = target.dataset.id;
      const resp = await apiFetch(`/admin/post/toggle-featured/${postId}`, {
        method: 'POST',
        csrf: true,
      });
      if (resp && resp.ok !== false) window.location.reload();
    ...
    if (target.classList.contains('translate-btn')) {
      ...
      try {
        ...
        const contentToTranslate = sourceEl ? sourceEl.value : '';
        const translatedContent = await translate(contentToTranslate);
        const targetEl = document.getElementById(contentTargetId);
        if (targetEl) targetEl.value = translatedContent;
      } catch (error) {
...
(function () {
  ...
  // Determine initial mode: prefer existing class (server-side / early script), else stored; default = light (ignore system preference)
  let initialMode = document.documentElement.classList.contains('dark-mode') ? 'dark' : null;
  if (!initialMode) {
    try {
      initialMode = localStorage.getItem('pp_theme');
    } catch (e) {}
  }
...
async function doPrefSync() {
  ...
  try {
    ...
    const allowPref = consent.categories
      ? Boolean(consent.categories.preferences)
      : consent === 'all';
    if (!allowPref) return;
    const theme = localStorage.getItem('pp_theme') || 'system';
    const lvl =
      localStorage.getItem('pandasWayLevel') || localStorage.getItem('pandasWayLevelAlt5') || '1';
    // Heuristik: user logged in? server injected flag via data attr on body maybe future; fallback: attempt and ignore 401
    try {
      await apiFetch('/api/user/preferences', {
        method: 'POST',
        json: { theme, pandas_way_level: lvl },
        csrf: true,
      });
    } catch (e) {
      if (!(e && e.message && e.message.includes('401')))
        console.warn('Pref Sync Fehler', e.message);
    }
  } catch (e) {
    console.warn('Pref Sync Exception', e);
  }
}
document.addEventListener('DOMContentLoaded', () => {
...
window.addEventListener('pp:consent', function (ev) {
  try {
    ...
    if (detail && detail.categories && detail.categories.preferences) {
      queuePrefSync();
    }
  } catch (e) {}
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (inlineSelect) {
    inlineSelect.addEventListener('click', () => {
      openMediaLibraryModal({
        filter: '__featuredCombo',
        returnObject: true,
        callback: (media) => {
          const hidden = document.getElementById('featured_image_id');
          hidden.value = media.id || '';
          document.getElementById('inlineFeaturedName').textContent = media.path
            ? media.path.split('/').pop()
            : media.filename;
          // Falls der seitliche Preview vorhanden ist, aktualisieren
          const preview = document.getElementById('featuredPreview');
          if (preview) {
            preview.src = media.path || '/uploads/' + media.filename;
            preview.classList.remove('d-none');
          }
          const wrapper = document.getElementById('featuredPreviewWrapper');
    ...
    btn.addEventListener('click', () => {
      openMediaLibraryModal({
        ...
        callback: (media) => {
          ...
          const hint = document.getElementById('noFeaturedHint');
  ...
  if (miniFilter) {
    miniFilter.addEventListener('click', (e) => {
      const btnEl = e.target.closest('button[data-cat]');
      if (!btnEl) return;
