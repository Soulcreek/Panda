...
async function ensurePostsColumns(){
  ...
  try {
    ...
    if(!have.has('site_key')){
      try { await pool.query('ALTER TABLE posts ADD COLUMN site_key VARCHAR(64) NOT NULL DEFAULT "default", ADD INDEX idx_posts_site (site_key)'); console.log('[POSTS] site_key ergänzt'); } catch(e){ console.warn('[POSTS] site_key add fail', e.code); }
    } else {
      try { await pool.query('ALTER TABLE posts ADD INDEX idx_posts_site (site_key)'); } catch(_){ }
    }
    // Adjust uniqueness: move from global slug uniqueness to per-site (site_key, slug). Drop old single-column index if exists.
    try { await pool.query('ALTER TABLE posts DROP INDEX slug'); } catch(_){ }
    try { await pool.query('ALTER TABLE posts DROP INDEX uq_posts_site_slug'); } catch(_){ }
    try { await pool.query('ALTER TABLE posts ADD UNIQUE INDEX uq_posts_site_slug (site_key, slug)'); } catch(e){ if(e.code!=='ER_DUP_ENTRY') console.warn('[POSTS] composite unique add warn', e.code); }
...
router.get('/posts', isEditor, async (req,res)=>{
  try { const [posts] = await pool.query('SELECT * FROM posts WHERE COALESCE(is_deleted,0)=0 AND site_key=? ORDER BY updated_at DESC',[req.siteKey]); res.render('editors_posts',{ title:'Beiträge', posts, archived:false }); }
...
router.post('/posts/new', isEditor, async (req,res)=>{
  ...
  async function ensureUniqueSlug(base){ let slug=base; let i=2; while(true){ try { const [rows]=await pool.query('SELECT id FROM posts WHERE slug=? AND site_key=? LIMIT 1',[slug, req.siteKey]); if(!rows.length) return slug; } catch(e){ const [rows]=await pool.query('SELECT id FROM posts WHERE slug=? LIMIT 1',[slug]); if(!rows.length) return slug; } slug = base + '-' + i++; } }
  ...
  try {
    ...
    await pool.query('INSERT INTO posts (title,slug,content,author_id,status,title_en,content_en,whatsnew,featured_image_id,published_at,tags,seo_title,seo_description,meta_keywords,site_key) VALUES (?,?,?,?, ?,?,?, ?,?,?, ?,?,?,?,?)',[title, slug, content||'', req.session.userId, status||'draft', title_en, content_en, wn, featured_image_id||null, pub, tags||null, seo_title||null, seo_description||null, meta_keywords||null, req.siteKey]);
...
router.post('/posts/edit/:id', isEditor, async (req,res)=>{
  ...
  await ensureRevisionsTable();
  function baseSlug(str){ return (str||'').toLowerCase().trim().replace(/[^a-z0-9\s-]/g,'').replace(/\s+/g,'-').replace(/-+/g,'-').replace(/^-|-$/g,'').slice(0,190) || 'post'; }
  async function ensureUniqueSlug(base, excludeId){ let slug=base; let i=2; while(true){ try { const [rows]=await pool.query('SELECT id FROM posts WHERE slug=? AND id<>? AND site_key=? LIMIT 1',[slug, excludeId, req.siteKey]); if(!rows.length) return slug; } catch(e){ const [rows]=await pool.query('SELECT id FROM posts WHERE slug=? AND id<>? LIMIT 1',[slug, excludeId]); if(!rows.length) return slug; } slug = base + '-' + i++; } }
  const wn = sanitizeWhatsNew(whatsnew);
  try {
    // Hole aktuelle Post um festzustellen ob Titel sich geändert hat und alter Slug behalten werden kann
  const [currentRows]=await pool.query('SELECT slug,title,content FROM posts WHERE id=? AND site_key=?',[req.params.id, req.siteKey]);
    ...
    const pub = published_at ? published_at.replace('T',' ') : null;
  await pool.query('UPDATE posts SET title=?,slug=?,content=?,status=?,title_en=?,content_en=?,whatsnew=?,featured_image_id=?,published_at=?,tags=?,seo_title=?,seo_description=?,meta_keywords=? WHERE id=? AND site_key=?',[title, slug, content||'', status||'draft', title_en, content_en, wn, featured_image_id||null, pub, tags||null, seo_title||null, seo_description||null, meta_keywords||null, req.params.id, req.siteKey]);
    res.redirect('/editors/posts');
  } catch(e){
