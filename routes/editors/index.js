...
// Reuse centralized auth to keep role checks consistent across app
...
router.get('/', isEditor, async (req, res) => {
  try {
    ...
    async function safeCount(table) {
      try {
        const [[r]] = await pool.query(`SELECT COUNT(*) c FROM ${table} WHERE site_key=?`, [
          siteKey,
        ]);
        return r.c;
      } catch (e) {
        if (e.code === 'ER_BAD_FIELD_ERROR') {
          ...
          if (['posts', 'media', 'podcasts'].includes(table)) {
            ...
            try {
              const [[r2]] = await pool.query(`SELECT COUNT(*) c FROM ${table} WHERE site_key=?`, [
                siteKey,
              ]);
              return r2.c;
            } catch (e2) {
          ...
          const [[r3]] = await pool.query(`SELECT COUNT(*) c FROM ${table}`);
          return r3.c;
        }
        throw e;
      }
    ...
    async function safeLatestPosts() {
      try {
        const [rows] = await pool.query(
          'SELECT id,title,updated_at,status FROM posts WHERE site_key=? ORDER BY updated_at DESC LIMIT 6',
          [siteKey]
        );
        return rows;
      } catch (e) {
        if (e.code === 'ER_BAD_FIELD_ERROR') {
          ...
          const [rows3] = await pool.query(
            'SELECT id,title,updated_at,status FROM posts ORDER BY updated_at DESC LIMIT 6'
          );
          return rows3;
        }
        throw e;
  ...
  // Observability: summary queries can be slow on cold caches
  console.log('[EDITORS DASHBOARD] summary start siteKey=', siteKey);
    ...
    const aiUsage = { used: 0, limit: 500, percent: 0 };
    try {
      try {
        const [r] = await pool.query(
          'SELECT SUM(calls) s FROM ai_usage WHERE site_key=? AND day BETWEEN (CURDATE() - INTERVAL 2 DAY) AND CURDATE()',
          [siteKey]
        );
        ...
        if (e.code === 'ER_BAD_FIELD_ERROR') {
          const [r2] = await pool.query(
            'SELECT SUM(calls) s FROM ai_usage WHERE day BETWEEN (CURDATE() - INTERVAL 2 DAY) AND CURDATE()'
          );
  ...
  // Log full stack to help diagnose transient DB issues (shared hosting)
    try {
      console.warn('[EDITORS DASHBOARD] Fehler', e.code, e && e.message, '\n', e && e.stack);
    } catch (_) {
      console.warn('[EDITORS DASHBOARD] Fehler (no stack available)', e);
    }
