...
async function fetchCategories(siteKey){
	try { const [cats] = await pool.query('SELECT slug,label FROM media_categories WHERE site_key=? ORDER BY label ASC',[siteKey]); return cats; } catch(e){ return []; }
}
...
router.post('/media/edit/:id', isEditor, async (req,res)=>{ const { name, alt_text, description, seo_alt, seo_description, meta_keywords, category }=req.body; try {
	...
	if(catSlug){
		try { const [c] = await pool.query('SELECT id,slug FROM media_categories WHERE (slug=? OR label=?) AND site_key=? LIMIT 1',[catSlug,catSlug, req.siteKey]); if(c.length){ catId=c[0].id; catSlug = c[0].slug || catSlug; } else { return res.status(400).send('Ungültige Kategorie'); } } catch(_){ }
	}
	await pool.query('UPDATE media SET name=?, alt_text=?, description=?, seo_alt=?, seo_description=?, meta_keywords=?, category_id=? WHERE id=? AND site_key=?',[name, alt_text, description, seo_alt, seo_description, meta_keywords, catId, req.params.id, req.siteKey]); res.redirect('/editors/media'); } catch(e){ res.status(500).send('Medien Update Fehler'); } });
...
router.post('/upload', isEditor, upload.array('mediaFiles',25), async (req,res)=>{ try {
	...
	for(const f of req.files){
		...
		if(catVal){ try { const [c] = await pool.query('SELECT 1 FROM media_categories WHERE (slug=? OR label=?) AND site_key=? LIMIT 1',[catVal,catVal, req.siteKey]); if(!c.length) catVal=null; } catch(_){ catVal=null; } }
		// Resolve category_id
