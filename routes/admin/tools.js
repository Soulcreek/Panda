...
// Access note: Admin tools here are operational diagnostics intended for editors/admins.
// Keep isAuth – do not restrict to isAdmin unless requirements change.
const { isAuth } = require('../../lib/auth');
const router = express.Router();

router.get(['/tools', '/tools/raw', '/tools/tables'], isAuth, async (req, res) => {
  const mode = req.path.endsWith('/raw')
    ? 'raw'
  ...
  try {
    ...
    if (mode === 'structured') {
      ...
      try {
        const [rows] = await pool.query(
          `SELECT id,title,COALESCE(is_deleted,0) as is_deleted,COALESCE(is_featured,0) as is_featured,COALESCE(status='published',0) as is_visible,published_at FROM posts ORDER BY id DESC LIMIT 200`
        );
        posts = rows;
        note(`Posts found: ${rows.length}`);
        if (!rows.length) note('Keine Posts gefunden (Tabelle leer?)');
      } catch (e) {
        note('Posts Query Fehler: ' + e.message);
      }
      try {
        const [rows] = await pool.query(
          `SELECT id,title,published_at,seo_title,seo_description FROM podcasts ORDER BY id DESC LIMIT 100`
        );
        podcasts = rows;
        note(`Podcasts found: ${rows.length}`);
        if (!rows.length) note('Keine Podcasts gefunden');
      } catch (e) {
        note('Podcasts Query Fehler: ' + e.message);
      }
      try {
        const [rows] = await pool.query(
          `SELECT id,title,slug,status,updated_at,is_template FROM advanced_pages ORDER BY updated_at DESC LIMIT 150`
        );
        pages = rows;
        note(`Pages found: ${rows.length}`);
        if (!rows.length) note('Keine Advanced Pages gefunden');
      } catch (e) {
        note('Advanced Pages Query Fehler: ' + e.message);
      }
      return res.render('admin_tools', {
        title: 'Admin Tools',
        mode,
        posts,
        podcasts,
    ...
    if (mode === 'raw') {
      ...
      try {
        ...
        note('SHOW TABLES Fehler: ' + e.message);
      }
      for (const t of tables) {
        try {
          ...
          rawData[t] = [{ error: 'Nicht lesbar' }];
          note(`Fehler beim Lesen von ${t}: ${e.message}`);
        }
      }
      return res.render('admin_tools', {
        title: 'Admin Tools',
    ...
    if (mode === 'tables') {
      ...
      if (tables.length) {
        try {
          const [rows] = await pool.query(
            `SELECT table_name,column_name,data_type,character_maximum_length,is_nullable,column_key,extra FROM information_schema.columns WHERE table_schema = DATABASE() ORDER BY table_name, ordinal_position`
          );
          note(`Schema rows found: ${rows.length}`);
          rows.forEach((r) => {
            (schemaByTable[r.table_name] = schemaByTable[r.table_name] || []).push(r);
          });
        } catch (e) {
          note('Information_schema Query Fehler: ' + e.message);
        }
      }
      return res.render('admin_tools_tables', {
    ...
    if (mode === 'tables')
      return res.status(500).render('admin_tools_tables', {
        title: 'Tabellen Übersicht',
        mode: 'tables',
        tables: [],
        schemaByTable: {},
    ...
    res.status(500).render('admin_tools', {
      title: 'Admin Tools',
      mode,
      posts: [],
      podcasts: [],
...
router.get('/tools/debug-json', isAuth, async (req, res) => {
  ...
  try {
    ...
    const [posts] = await pool.query(
      `SELECT id,title,COALESCE(is_deleted,0) as is_deleted,COALESCE(is_featured,0) as is_featured,COALESCE(status='published',0) as is_visible,published_at FROM posts ORDER BY id DESC LIMIT 5`
    );
    note(`✅ Posts query OK: ${posts.length} rows`);

    note('Testing podcasts query...');
    const [podcasts] = await pool.query(
      `SELECT id,title,published_at FROM podcasts ORDER BY id DESC LIMIT 5`
    );
...
router.get('/tools/prompt-tester', isAuth, (req, res) => {
  res.render('admin_tool_prompt_tester', { title: 'AI Prompt Tester', result: null, error: null });
});
router.post('/tools/prompt-tester', isAuth, async (req, res) => {
  const { prompt = '', model = 'fast', temperature = '0.7' } = req.body || {};
  const t = prompt.trim();
  if (!t)
    return res.render('admin_tool_prompt_tester', {
  ...
  let rawResp = '',
    parse_error = null;
  try {
    const { callGeminiRaw } = require('../../lib/aiHelpers');
  ...
  res.render('admin_tool_prompt_tester', {
    title: 'AI Prompt Tester',
    result: rawResp,
    error: parse_error,
  });
...
router.get('/tools/diag', isAuth, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: e.message, env_DB_NAME: process.env.DB_NAME || null, debug });
  }
});
...
router.post('/generate-alt-text', isAuth, async (req, res) => {
  try {
    ...
    const description =
      'Automatisch erstellte Beschreibung für ' + (base.substring(0, 200) || 'die Datei');
  ...
  try {
    ...
    for (const n of names.slice(0, 500)) {
      try {
        ...
        list.push({ name: n, error: e.message });
      }
    }
