...
router.get('/consent-events', isAdmin, async (req,res)=>{
  try{
    ...
    if(pool){
      try{
        const [r] = await pool.query('SELECT id, categories, meta, created_at FROM consent_events ORDER BY created_at DESC LIMIT 100');
        rows = r || [];
      }catch(e){ console.warn('[admin/consent] DB fetch failed', e.message); }
    }
...
router.post('/consent-events/purge', isAdmin, async (req,res)=>{
  try{
    ...
    await pool.query('DELETE FROM consent_events WHERE created_at < NOW() - INTERVAL ? DAY', [days]);
    return res.redirect('/admin/consent-events');
  }catch(e){ console.error('Purge failed', e); return res.status(500).send('Purge failed'); }
});
