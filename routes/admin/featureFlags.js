...
router.post('/api/feature-flags', isAdmin, async (req, res) => {
  ...
  if (!flag_key || !flag_key.trim())
    return res.apiError(400, { error: 'flag_key fehlt', code: 'FLAG_KEY_MISSING' });
  const key = flag_key
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9_-]/g, '')
    .slice(0, 100);
  const ok = await upsertFlag(
    req.siteKey,
    key,
    enabled === '1' || enabled === 1 || enabled === true,
    variant ? variant.trim().slice(0, 64) : null,
    description ? description.trim().slice(0, 255) : null
  );
  if (!ok)
    return res.apiError(500, { error: 'Speichern fehlgeschlagen', code: 'FLAG_UPSERT_FAIL' });
  const flags = await listFlags(req.siteKey);
  res.apiOk({ flags });
});
