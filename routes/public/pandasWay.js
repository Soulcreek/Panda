...
router.get('/pandas-way-alt1', (req, res) => res.render('pandas_way_alt1', { title: "Panda's Way – ALT 1" }));
router.get('/pandas-way-alt2', (req, res) => res.render('pandas_way_alt2', { title: "Panda's Way – ALT 2" }));
router.get('/pandas-way-alt3', (req, res) => res.render('pandas_way_alt3', { title: "Panda's Way – ALT 3" }));
router.get('/pandas-way-alt4', (req, res) => res.render('pandas_way_alt4', { title: "Panda's Way – ALT 4" }));
...
router.get('/pandas-way-alt5', async (req, res) => {
  try {
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS timeline_site_config (site_key VARCHAR(64) PRIMARY KEY, level_count INT NOT NULL DEFAULT 3, design_theme VARCHAR(32) NOT NULL DEFAULT 'glass', updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;`);
    ...
    if(!rows.length){
      ...
      for(const s of seed){ await pool.query('INSERT INTO timeline_entries (site_key, position, level, title, phase, content_html) VALUES (?,?,?,?,?,?)',[ 'pandas_way_5', s.position, s.level, s.title, s.phase, s.html]); }
    ...
    if(!cfgRows.length){ await pool.query('INSERT INTO timeline_site_config (site_key, level_count, design_theme) VALUES (?,?,?)',[ 'pandas_way_5', 3, 'glass']); ;[cfgRows] = await pool.query('SELECT * FROM timeline_site_config WHERE site_key=?',['pandas_way_5']); }
    const siteCfg = cfgRows[0];
    for(let i=1;i<=siteCfg.level_count;i++){ const [exists] = await pool.query('SELECT id FROM timeline_levels WHERE site_key=? AND level_index=?',[ 'pandas_way_5', i]); if(!exists.length){ await pool.query('INSERT INTO timeline_levels (site_key, level_index, title) VALUES (?,?,?)',[ 'pandas_way_5', i, 'Level '+i]); } }
