...
setInterval(()=>{ for(const k of Object.keys(rateState)){ if(Date.now()-rateState[k].ts > 60*1000) delete rateState[k]; } }, 30*1000);
function rateLimit(req,res,next){
  const ip = req.ip || req.connection.remoteAddress || 'anon';
  let entry = rateState[ip];
  if(!entry || (Date.now()-entry.ts) > 60*1000){ entry = { ts: Date.now(), c:0 }; rateState[ip]=entry; }
  entry.c++;
  if(entry.c > LIMIT){ return res.status(429).json({ error:'Rate Limit', code:'RATE_LIMIT', detail:'Zu viele Anfragen, warte eine Minute.' }); }
  next();
}
...
router.get('/api/v1/posts', async (req,res)=>{
  try {
    ...
    const [rows] = await pool.query(`SELECT p.id, p.slug, p.title, p.seo_title, p.whatsnew, p.published_at, p.created_at, p.meta_keywords, m.path AS featured_image_path
      FROM posts p
  ...
  } catch(e){ res.status(500).json({ error:'Posts Fetch Fehler', code:'POSTS_FETCH', detail:e.message }); }
});
...
router.get('/api/v1/posts/:idOrSlug', async (req,res)=>{
  try {
    ...
    const [rows] = await pool.query(`SELECT p.id, p.slug, p.title, p.content, p.seo_title, p.seo_description, p.whatsnew, p.published_at, p.created_at, p.meta_keywords, m.path AS featured_image_path
      FROM posts p
      LEFT JOIN media m ON p.featured_image_id=m.id
      WHERE ${where} AND p.status='published' AND (p.published_at IS NULL OR p.published_at<=NOW()) AND p.is_deleted=0 LIMIT 1`, [idOrSlug]);
    ...
    res.json(post);
  } catch(e){ res.status(500).json({ error:'Post Fetch Fehler', code:'POST_FETCH', detail:e.message }); }
});
...
router.get('/api/v1/media', async (req,res)=>{
  ...
  } catch(e){ res.status(500).json({ error:'Media Fetch Fehler', code:'MEDIA_FETCH', detail:e.message }); }
});
