  ...
  try {
    ...
    try {
      ...
      const [postCount] = await pool.query(
        `SELECT COUNT(*) as count FROM posts WHERE status='published' AND is_deleted=0`
      );
...
router.get('/blog', async (req, res) => {
  try {
    ...
    const [countRows] = await pool.query(
      `SELECT COUNT(*) as c FROM posts p WHERE p.status='published' AND (p.published_at IS NULL OR p.published_at<=NOW()) AND p.is_deleted=0`
    );
...
router.get('/blog/tag/:tag', async (req, res) => {
  ...
  try {
    const sql = `SELECT p.*, m.path as featured_image_path
               FROM posts p
               LEFT JOIN media m ON p.featured_image_id = m.id
...
router.get('/search', async (req, res) => {
  ...
  const like = '%' + q + '%';
  try {
    const base = `SELECT p.*, m.path as featured_image_path FROM posts p LEFT JOIN media m ON p.featured_image_id = m.id WHERE p.status='published' AND (p.published_at IS NULL OR p.published_at <= NOW()) AND p.is_deleted=0`;
    const whereParts = ['(p.title LIKE ? OR p.content LIKE ?'];
...
router.get('/api/blog/:id', async (req, res) => {
  try {
    ...
    res.json({
      id: post.id,
      title: post.title,
      content: post.content || '<p>(Kein Inhalt)</p>',
      image_path: post.featured_image_path || null,
      tags: post.tags || '',
...
router.get('/blog.rss', async (req, res) => {
  try {
    const now = Date.now();
    if (blogRssCache.xml && now - blogRssCache.ts < 5 * 60 * 1000) {
      setFeedCache(res);
      res.set('Content-Type', 'application/rss+xml; charset=utf-8');
      return res.send(blogRssCache.xml);
    }
    const [posts] = await pool.query(
      `SELECT p.id, p.title, p.content, p.created_at, p.published_at, p.slug FROM posts p WHERE p.status='published' AND (p.published_at IS NULL OR p.published_at<=NOW()) AND p.is_deleted=0 ORDER BY COALESCE(p.published_at, p.created_at) DESC LIMIT 50`
    );
    ...
    const items = posts
      .map((r) => {
        const body = (r.content || '').replace(/<script[\s\S]*?<\/script>/gi, '');
        return `<item><title><![CDATA[${r.title}]]></title><link>${siteUrl}/blog#post-${r.id}</link><guid isPermaLink="false">post-${r.id}</guid><pubDate>${new Date(r.published_at || r.created_at).toUTCString()}</pubDate><description><![CDATA[${body.slice(0, 1200)}]]></description></item>`;
      })
      .join('\n');
    ...
    blogRssCache = { xml: rss, ts: now };
    setFeedCache(res);
    res.set('Content-Type', 'application/rss+xml; charset=utf-8');
    res.send(rss);
  } catch (e) {
    console.error('Blog RSS Fehler', e);
    res.status(500).send('Blog RSS Fehler');
  }
});
...
router.get('/blog/:slug', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT p.*, m.path AS featured_image_path FROM posts p LEFT JOIN media m ON p.featured_image_id=m.id WHERE p.slug=? AND p.status='published' AND COALESCE(p.is_deleted,0)=0 LIMIT 1`,
      [slug]
    );
    if (!rows.length)
      return res.status(404).render('partials/error_404', { title: 'Nicht gefunden' });
    const post = rows[0];
    let related = [];
    if (post.tags) {
      ...
      if (firstTag) {
        const [rel] = await pool.query(
          `SELECT id,title,slug,whatsnew,published_at FROM posts WHERE id<>? AND tags LIKE ? AND status='published' AND COALESCE(is_deleted,0)=0 ORDER BY published_at DESC LIMIT 5`,
          [post.id, '%' + firstTag.trim() + '%']
        );
        related = rel;
      }
    }
    ...
    res.render('blog_detail', {
      ...
      seo: {
        title: post.seo_title || post.title,
        description: post.seo_description || post.whatsnew || '',
    ...
    console.error('Slug route Fehler', e);
