...
function rateLimitLogin(req, res, next) {
  try {
    ...
    if (arr.length >= maxAttempts) {
      ...
      return res
        .status(429)
        .render('login', { title: 'Login', error: 'Zu viele Versuche. Bitte später erneut.' });
    }
    arr.push(now);
    __loginAttempts.set(ip, arr);
  } catch (_) {
    /* ignore */
  }
  next();
...
async function ensureUserPreferencesTable() {
  try {
    ...
    console.error('Prefs Table Fehler:', e.message);
  }
}
...
async function saveUserPreferences(userId, { theme, pandas_way_level }) {
  ...
  let lvl = parseInt(pandas_way_level, 10);
  if (!lvl || lvl < 1 || lvl > 10) lvl = 1; // 1..10 plausible cap
  await pool.query(
    `INSERT INTO user_preferences (user_id, theme, pandas_way_level) VALUES (?,?,?)
    ON DUPLICATE KEY UPDATE theme=VALUES(theme), pandas_way_level=VALUES(pandas_way_level), updated_at=CURRENT_TIMESTAMP`,
    [userId, finalTheme, lvl]
  );
...
async function ensureUserSoftDeleteColumns() {
  try {
    await pool.query('ALTER TABLE users ADD COLUMN is_deleted TINYINT(1) NOT NULL DEFAULT 0');
  } catch (_) {}
}
...
router.use(async (req, res, next) => {
  ...
  if (req.session.userId) {
    try {
      res.locals.userPrefs = await getUserPreferences(req.session.userId);
    } catch (_) {}
  }
  next();
});
...
router.get('/account', requireLogin, async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      'SELECT id, username, is_deleted, created_at, updated_at FROM users WHERE id=?',
      [req.session.userId]
    );
    ...
    console.error('Account Load Fehler', e);
    res.status(500).send('Fehler Account');
  }
});
...
router.get('/account/export', requireLogin, async (req, res) => {
  try {
    ...
    const exportObj = {
      ...
      ai_calls: 'Aggregierte AI Logs sind anonymisiert / nicht usergebunden',
    };
    res.setHeader('Content-Type', 'application/json; charset=utf-8');
    res.setHeader(
      'Content-Disposition',
      'attachment; filename="account-export-' + req.session.userId + '.json"'
    );
    res.send(JSON.stringify(exportObj, null, 2));
  } catch (e) {
    console.error('Export Fehler', e);
    res.status(500).send('Export Fehler');
  }
});
...
router.post('/account/delete', requireLogin, async (req, res) => {
  try {
    ...
    await pool.query(
      'UPDATE users SET username=?, password=REPEAT("x",60), is_deleted=1 WHERE id=?',
      [anonUser, uid]
    );
    try {
      await pool.query('DELETE FROM user_preferences WHERE user_id=?', [uid]);
    } catch (_) {}
    req.session.destroy(() => {});
    ...
    res.status(500).send('Löschung fehlgeschlagen');
  }
});
...
router.post('/api/user/preferences', async (req, res) => {
  if (!req.session.userId)
    return res.apiError(401, { error: 'Nicht eingeloggt', code: 'USER_PREFS_UNAUTH' });
  try {
...
router.get('/login', (req, res) => {
  if (req.session.userId) {
    const target =
      req.query.redirect && /^\/[a-zA-Z0-9/_-]+$/.test(req.query.redirect)
        ? req.query.redirect
        : '/editors';
...
router.post('/login', rateLimitLogin, async (req, res) => {
  ...
  try {
    ...
    if (!rows.length)
      return res.status(401).render('login', { title: 'Login', error: 'Ungültige Zugangsdaten' });
    const user = rows[0];
    ...
    // Ensure session is saved in the store before redirecting to avoid race
    // where subsequent requests don't see the session (causing a redirect loop)
    req.session.save((err) => {
      if (err) console.warn('[LOGIN] session save failed', err && err.message);
      // small debug log to confirm server-side session state (no secrets printed)
      try {
        const safe = {};
        for (const k of Object.keys(req.session || {}))
          safe[k] = typeof req.session[k] === 'object' ? '[object]' : String(req.session[k]);
        console.log('[LOGIN] user %s session saved, sid=%s', user.id, req.sessionID);
        console.log('[LOGIN] session snapshot:', safe);
      } catch (_) {}
      let target = '/editors';
...
router.post('/register', async (req, res) => {
  ...
  if (!username || !password)
    return res
      .status(400)
      .render('register', { title: 'Registrieren', error: 'Alle Felder erforderlich' });
  try {
    const hash = await bcrypt.hash(password, 12);
    // First user becomes admin automatically
    let role = 'editor';
    try {
      const [[cnt]] = await pool.query('SELECT COUNT(*) c FROM users');
      if (cnt.c === 0) role = 'admin';
    } catch (_) {}
