...
router.get('/purview', async (req, res) => {
  try {
    ...
    const [aggregates] = await pool.query(
      'SELECT payload, generated_at FROM purview_aggregates ORDER BY generated_at DESC LIMIT 1'
    );
    let data = {};
  ...
  const glossary = [
    {
      term: 'Data Catalog',
      definition:
        'A centralized repository that contains metadata about your data assets, making them discoverable and understandable.',
    },
    {
      term: 'Data Classification',
      definition:
        'The process of organizing data by relevant categories so that it can be used and protected more efficiently.',
    },
    {
      term: 'Data Lineage',
      definition:
        'The tracking of data flow from its origin to destination, including all transformations along the way.',
    },
    {
      term: 'Sensitivity Labels',
      definition:
        "Tags that classify and protect your organization's data based on its sensitivity level.",
    },
    {
  ...
  const faqs = [
    {
      ...
      answer:
        'Microsoft Purview is a comprehensive data governance solution that helps organizations discover, classify, and protect their data across on-premises, cloud, and hybrid environments.',
    },
    {
      question: 'How does it help with compliance?',
      answer:
        'Purview provides automated data discovery, classification, and policy enforcement to help meet regulatory requirements like GDPR, HIPAA, and industry standards.',
    },
    {
      question: 'Can I see what data we have?',
      answer:
        'Yes, the data catalog provides a searchable inventory of your data assets with metadata, schema information, and usage patterns.',
    },
    {
      question: 'How do I tag content for better discoverability?',
      answer:
        "Use the media library and content editor to add descriptive tags, categories, and metadata that align with your organization's taxonomy.",
    },
  ];
...
router.get('/api/public/purview', async (req, res) => {
  try {
    ...
    if (rows && rows[0])
      return res.json({ ok: true, data: rows[0].payload, generated_at: rows[0].generated_at });
    // fallback sample response when no aggregates exist
    const sample = {
      counts: { posts: 0, podcasts: 0, media: 0 },
      last_run: null,
      notes: 'No aggregates available yet',
    };
