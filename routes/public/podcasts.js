...
router.get('/podcasts', async (req, res) => {
  try {
    const [podcasts] = await pool.query(
      `SELECT id, slug, title, description, audio_url, published_at, seo_title, seo_description FROM podcasts ORDER BY published_at DESC`
    );
...
router.get('/podcasts/:slugOrId', async (req, res) => {
  try {
    ...
    const [rows] = isId
      ? await pool.query(
          'SELECT id, slug, title, description, audio_url, published_at, seo_title, seo_description, meta_keywords FROM podcasts WHERE id=? LIMIT 1',
          [key]
        )
      : await pool.query(
          'SELECT id, slug, title, description, audio_url, published_at, seo_title, seo_description, meta_keywords FROM podcasts WHERE slug=? LIMIT 1',
          [key]
        );
    if (!rows.length)
      return res.status(404).render('partials/error_404', { title: 'Episode nicht gefunden' });
    const ep = rows[0];
    // Canonical redirect if accessed by id but slug exists
    if (isId && ep.slug) {
      return res.redirect(301, '/podcasts/' + ep.slug);
    }
    const meta = {
      title: ep.seo_title || ep.title,
      description: ep.seo_description || (ep.description || '').slice(0, 160),
      keywords: ep.meta_keywords || '',
    };
...
router.get('/podcast.rss', async (req, res) => {
  try {
    ...
    const [episodes] = await pool.query(
      `SELECT id, slug, title, description, audio_url, published_at FROM podcasts ORDER BY published_at DESC LIMIT 100`
    );
    ...
    const lastBuild = episodes.length
