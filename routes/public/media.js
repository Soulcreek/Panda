...
router.get('/media', async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      'SELECT id, name, type, path FROM media ORDER BY uploaded_at DESC LIMIT 200'
    );
    let itemsHtml;
    if (rows && rows.length) {
      itemsHtml = rows
        .map(
          (r) =>
            `<li><a href="${r.path}" target="_blank" rel="noopener">${r.name}</a> <small>(${r.type || ''})</small></li>`
        )
        .join('');
    } else {
      const fsItems = listUploadFiles();
      itemsHtml = fsItems.length
        ? fsItems
            .map(
              (r) =>
                `<li><a href="${r.path}" target="_blank" rel="noopener">${r.name}</a> <small>(${r.type || ''})</small></li>`
    ...
    const fsItems = listUploadFiles();
    const itemsHtml = fsItems.length
      ? fsItems
          .map(
            (r) =>
              `<li><a href="${r.path}" target="_blank" rel="noopener">${r.name}</a> <small>(${r.type || ''})</small></li>`
