  ...
  try {
    ...
    try {
      if (metrics && typeof metrics.inc === 'function')
        metrics.inc('consent_events_total', 1, {
          prefs: !!body.categories.preferences,
    ...
    if (pool) {
      try {
        await pool.query('INSERT INTO consent_events (categories, meta) VALUES (?, ?)', [
          JSON.stringify(body.categories),
          JSON.stringify({ ts: body.ts || null }),
        ]);
      } catch (e) {
        console.warn('[consent] DB insert failed', e.message);
      }
    }

    return res.json({ ok: true });
  } catch (e) {
    console.error('Consent endpoint error', e);
