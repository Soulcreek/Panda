...
router.post('/api/track', express.json(), (req,res)=>{
  try{
    ...
    try{ if(metrics && typeof metrics.inc === 'function') metrics.inc('events_tracked_total', 1, { event }); }catch(e){}
    return res.json({ ok:true });
  }catch(e){ console.error('Track error', e); return res.status(500).json({ ok:false, error: e.message }); }
});
