...
(async function(){
  ...
  if(!cfg.user || !cfg.password || !cfg.database){
    console.error('Missing required parameters. Provide --user, --password, --database or set DB_USER/DB_PASSWORD/DB_NAME env vars.');
    process.exit(2);
  }

  const out = { host: cfg.host, port: cfg.port, user: cfg.user, database: cfg.database, timestamp: (new Date()).toISOString(), results: {} };
  let conn;
  try {
    conn = await mysql.createConnection({ host: cfg.host, port: cfg.port, user: cfg.user, password: cfg.password, database: cfg.database, connectTimeout: 10000 });
    // basic identity
    try {
      const [rows] = await conn.query('SELECT CURRENT_USER() AS current_user, DATABASE() AS database_name');
      out.results.identity = rows && rows[0] ? rows[0] : null;
    } catch(e){ out.results.identity_error = e.message; }
    ...
    } catch(e){ out.results.show_tables_error = e.message; }
    ...
    } catch(e){ out.results.info_schema_error = e.message; }
    ...
    } catch(e){ out.results.posts_count_error = e.message; }
  ...
  try { fs.mkdirSync(path.dirname(outPath), { recursive:true }); fs.writeFileSync(outPath, JSON.stringify(out,null,2),'utf8'); console.log('Wrote:', outPath); } catch(e){ console.error('Could not write output file:', e.message); }
