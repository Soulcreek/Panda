...
async function getAppliedMigrations(conn){
  const [rows] = await conn.query('SELECT filename,checksum,applied_at FROM schema_migrations ORDER BY id ASC');
  const map = new Map();
...
async function run(){
  ...
  if(cfg.help || !cfg.database){
    console.log('Usage: node scripts/run_migrations.js --database=DB_NAME [--host=] [--user=] [--password=] [--dir=./migrations] [--dry-run]');
    console.log('Environment variables used if not specified: DB_HOST, DB_PORT, DB_USER, DB_PASSWORD, DB_NAME, MIGRATIONS_DIR');
    process.exit(cfg.help?0:2);
  }

  if(!fs.existsSync(cfg.dir)){ console.error('Migrations directory not found:', cfg.dir); process.exit(1); }
  const files = fs.readdirSync(cfg.dir).filter(f=>f.endsWith('.sql')).sort();
  if(!files.length){ console.log('No .sql files in', cfg.dir); process.exit(0); }

  let conn;
  try {
    conn = await mysql.createConnection({ host: cfg.host, port: cfg.port, user: cfg.user, password: cfg.password, database: cfg.database, multipleStatements: true, connectTimeout: 10000 });
    ...
    for(const file of files){
      ...
      if(!sql){ console.log('[skip] empty file', file); continue; }
      const checksum = sha256(sql);
      const already = applied.get(file);
      if(already){
        if(already.checksum !== checksum){
          console.error('[error] checksum mismatch for', file, '\n - applied checksum:', already.checksum, '\n - current checksum:', checksum, '\n -> Manual intervention required (rename file or adjust).');
          process.exit(1);
        }
