...
(async function(){
  try {
    ...
    if(!files.length){ console.log('No image files found in uploads.'); process.exit(0); }

    let hasSharp = false; let sharp;
    try { sharp = require('sharp'); hasSharp=true; } catch(e){ hasSharp=false; }
    ...
    for(const f of files){
      ...
      try {
        if(hasSharp){
          ...
          if(st.size < 1024*1024){ // copy if <1MB
